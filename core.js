...
function getExcluded() { 
  ...
  if (!hasGoal('女性线条')) {
    const erotic = [
      '蛙式臀桥', '蛙式臀冲', '跪姿挺髋后仰', '深度深蹲摇摆', '跪姿下沉触踵', '宽距深蹲弹震', '俯卧青蛙翘尾', 
      '肩垫高极限臀推', '跪姿股四极限后仰', '仰卧风车画腿', '跪姿狂野摇摆', '跪姿骨盆画八字', '四足爬行姿控', 
      '跪姿后踢抬腿', '跪姿俯身挺身', '四足跪姿髋外展', '四足爬行姿态', '仰卧魅态桥', '触觉连结发力', 
      '靠墙倒挂大V字', '仰卧折叠犁式', '靠墙九十度前屈', '反绑式跪姿前倾', '仰卧牛面式', '仰卧快乐婴儿式', 
      '青蛙趴', '小狗伸展式 (融心式)', '仰卧大V字开合', '池边悬浮分腿', '深水螺旋下沉', '池畔仰卧后仰'
    ];
...
const DB = {
  ...
  hamglutes: [
    ...
    { n: '蛙式臀冲', eq: ['无器材', '弹力带', '健身房全套'], muscle: ['臀', '内收肌'], diff: 1, note: '仰卧脚心相对、双膝极大敞开，连续将骨盆推向最高点并挤压' },    { n: '深度深蹲摇摆', eq: ['无器材', '健身房全套'], muscle: ['下肢', '髋外旋'], diff: 1, note: '极宽距深蹲到底，手肘撑开双膝，缓慢左右转移重心，深度唤醒腹股沟', u: '秒' },
...
// ══ Curve Splits (女性线条 — 极致女性线条: 圆臀+宽胯+丰满大腿+蛮腰) ══
const CURVE_SPLITS = {
  2: [
...
const TIPS = {
  ...
  '女性线条': '目标是极致女性线条：圆翘臀(臀推/硬拉)+宽胯(臀中肌外展)+丰满大腿(股四/腘绳/内收都练、堆围度)+极小蛮腰。臀和大腿都要渐进加重、长肉长围度，大腿要有肉感；练腿别一味只练外侧股四显"壮"，多兼顾腘绳/内收/臀腿衔接让大腿丰满柔顺。腰反过来——只做真空吸/平板收紧，绝不做负重转体/侧屈(会把腰练厚练方)，靠"胯臀大、腰细"的对比拉满沙漏感。上肢几乎不练以免变宽。吃要够(蛋白每公斤1.6-2g+轻微热量盈余)才能把臀腿喂大；腰围靠真空吸+控糖控总量+有氧维持，不靠饿。'
};
...
function applyGoalToggle(goal) {
  const EXCLUSIVE = ['倒三角矫正', '翘臀美背', '女性线条']; // 整套预设方案，不与其他目标组合
  ...
  S.goal = goals.join('+');
  if (hasGoal('倒三角矫正') || hasGoal('臀腿塑形') || hasGoal('翘臀美背') || hasGoal('女性线条')) {
    S.focus = ['下肢'];
...
function pickExercises(split, excluded) {
  ...
  split.groups.forEach(grp => {
    ...
    if ((hasGoal('臀腿塑形') || hasGoal('倒三角矫正') || hasGoal('女性线条')) && grp === 'core') {
  ...
  if (!split.groups.includes('cardio') && S.dur >= 45) {
    ...
    if (hasGoal('臀腿塑形') || hasGoal('倒三角矫正') || hasGoal('翘臀美背') || hasGoal('女性线条')) {
    ...
    if (cPool.length) {
      ...
      const cardioNote = (hasGoal('臀腿塑形') || hasGoal('倒三角矫正') || hasGoal('翘臀美背') || hasGoal('女性线条')) ? '臀腿有氧收尾 — 优先攀爬机/骑行机，保持对下肢的刺激' : '薄肌有氧收尾 — 保持心率120-140，帮助肌肉拉长';
...
function genPlan(isRecalibrate = false, preserveFuture = false) {
  ...
  const splits = hasGoal('女性线条') ? (CURVE_SPLITS[gymPerWeek] || CURVE_SPLITS[3]) :
...
const EX_DETAIL = {
  ...
  '蛙式臀冲': { muscles: ['臀大肌', '内收肌'], steps: ['仰卧，双脚脚心相对，双膝向两侧极度敞开（尽可能贴地）', '依靠臀部发力，将骨盆向上推至最高点', '在顶端用力收紧臀大肌，骨盆保持敞开状态'], tips: ['深度刺激臀肌的同时，要求内收肌和骨盆的极度放松与打开', '用臀发力，不要过度反弓腰椎'], mistakes: ['膝盖在发力时向内合拢', '用腰部代偿'] },  '跪姿躯干波浪': { muscles: ['脊柱', '核心'], steps: ['四足跪姿，手臂前伸', '下巴与胸腔贴近地面，像猫一样向前滑动', '重心前移后，拱起背部（像猫式）连贯地退回原位'], tips: ['强调脊柱如波浪般的连贯起伏', '动作可以极其缓慢，培养深层的身体觉知和控制力'], mistakes: ['动作僵硬断层', '只动脖子不动胸椎'] },
  ...
  '小狗伸展式 (融心式)': { muscles: ['上背部', '胸大肌'], steps: ['四足跪姿开始，保持臀部在膝盖正上方高高翘起', '双手向前滑行，让胸部、腋下和下巴完全贴伏、融化在地面上', '完全顺从重力，深长呼吸'], tips: ['这是一个极度敞开胸腔与展现放松感的体式', '若颈椎不适，可将额头垫高'], mistakes: ['臀部过度前移或后坐', '手臂没有完全伸展'] },  '仰卧牛面式': { muscles: ['臀中肌', '梨状肌'], steps: ['仰卧，双腿在空中紧紧交叉重叠（如大腿打结）', '双手抓住两只脚的脚背或脚踝，用力向下压向地板'], tips: ['身体有一种强烈的"被紧紧束缚"的错觉，臀外侧拉伸感极强', '保持下背贴地'], mistakes: ['头部抬起离开地面', '拉拽方向错误'] },
  ...
  '仰卧风车画腿': { muscles: ['下腹部', '髂腰肌', '大腿内收肌'], steps: ['仰卧，核心收紧，双腿绷直脚背（想象穿着15cm高跟鞋）', '一条腿带着另一条腿在空中像折扇一样，画出极其夸张、缓慢的巨大半圆', '双腿交替在空中交织、画圈'], tips: ['不仅是对下腹部极其严苛的考验，更是对双腿线条和胯部极其放肆的展示', '越慢越具有视觉冲击力'], mistakes: ['脚背松懈导致毫无美感', '用腰部代偿，下背离开地面'] },  '跪姿下沉触踵': { muscles: ['股四头肌离心', '臀大肌'], steps: ['高跪姿起手，核心和臀部收紧', '大腿前侧发力控制，极其缓慢地让臀部向后下方沉，直到轻轻擦过脚后跟', '瞬间发力向前向上挺出骨盆，回到起始高跪姿'], tips: ['慢落快起，在极度克制后爆发出强烈的视觉侵略性', '始终保持臀部肌肉处于紧张状态'], mistakes: ['完全放松坐到了脚后跟上失去张力'] },  '弹力带螃蟹步': {
    muscles: ['臀中肌'],
