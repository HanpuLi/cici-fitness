...
const EX_DETAIL = new Proxy({}, {
  get: function(target, name) {
    // 自动为所有动作生成详情，这样无需把几百个动作硬编码在此处。
    // 我们提取其在 DB 中的配置，并用通用提示填充，特殊动作可在 target 中覆写
    if(target[name]) return target[name];
    
    for (const cat in DB) {
      const ex = DB[cat].find(e => e.n === name);
      if (ex) {
        return {
          muscles: ex.muscle,
          steps: ['准备姿势：保持核心收紧，调整呼吸', '发力阶段：感受目标肌群收缩，控制速度', '动作过程：' + ex.note, '回放阶段：缓慢控制离心，切勿自由落体'],
          tips: ['使用器械时，先调整座位高度适应身型', '若感到关节不适，立刻减轻重量或停止'],
          mistakes: ['憋气（应在发力时呼气，下放时吸气）', '借助惯性甩动身体']
        };
      }
    }
    return null;
...
EX_DETAIL['杠铃卧推']={muscles:['胸大肌','三角肌前束','肱三头肌'],steps:['躺在卧推凳上，眼睛位于杠铃正下方','握距略宽于肩，拇指环握','深吸气，将杠铃下放至乳头位置','呼气，用力推起至起始位'],tips:['全程五点接触：头/上背/臀/双脚','下背可轻微弓起，不要过度后仰'],mistakes:['握距过宽伤肩','闭眼握（大拇指不环握）危险']};
EX_DETAIL['深蹲']={muscles:['股四头肌','臀大肌','腘绳肌'],steps:['站距与肩同宽，脚尖微外八','深吸气收腹，脊柱中立','下蹲至大腿平行，膝盖对准脚尖','蹬地站起'],tips:['重心在脚中央偏后'],mistakes:['膝盖内扣','脚跟离地']};
EX_DETAIL['罗马尼亚硬拉']={muscles:['腘绳肌','臀大肌','竖脊肌'],steps:['站直，杠铃在大腿前方','膝盖微屈锁定，臀部后推','沿腿下滑','驱动臀部向前站起'],tips:['杠铃全程贴近腿部'],mistakes:['弓背','膝盖过多弯曲']};
EX_DETAIL['引体向上']={muscles:['背阔肌','肱二头肌'],steps:['正手宽握握把','挺胸，肩胛骨下沉','拉至锁骨位置','缓慢下放'],tips:['想象用肘部向下向后拉'],mistakes:['借助身体摆动']};
EX_DETAIL['杠铃推举']={muscles:['三角肌前束','中束','肱三头肌'],steps:['杠铃在锁骨位置，握距略宽于肩','推至头顶','顶端手臂伸直','缓慢下放至锁骨'],tips:['不要过度后仰'],mistakes:['腰部过度后仰']};
EX_DETAIL['传统硬拉']={muscles:['后链全部'],steps:['杠铃在胫骨前，脚与髋同宽','背中立，腿推地启动','杠铃贴身上拉至站直','控制回放'],tips:['动作之王，量力而行'],mistakes:['弓背（最危险）']};
EX_DETAIL['臀推']={muscles:['臀大肌','腘绳肌','竖脊肌'],steps:['肩胛骨靠凳子边缘','双脚踩地膝盖90°','推髋向上','缓慢下放'],tips:['顶端夹臀'],mistakes:['腰部代偿']};
